...
Circuits: `post_key`, `shuffle`, `share` in `contracts/deal.compact` (v0: cryptographic core, no table bookkeeping yet).
...
- Product-argument shuffle (same day, machine and load): `shuffle` is k=16 (65,536 rows) with 2,199 IR instructions, against k=17 and 27,366; prover key 67 MB against 135 MB; key generation for the three circuits 31 s and 3.3 GB peak. Wasm proving under Bun: 82 s cold, 77 s warm, from 134 and 114 s; proof still 4,229 bytes; RSS 1.0 GB. Components measured on their own: the 104 multiplications k=15, the permutation argument k=13, the chained Poseidon over the deck k=12, the products k=12. Two prover facts: `transientHash` over a value containing points compiles to IR the wasm prover rejects ("unreachable"), so the circuit hashes coordinates through `jubjubPointX` and `jubjubPointY`; and proving time fell less than the row count (0.7x time for 0.5x rows), so a per-proof cost that does not scale with rows is significant at this size. Six sequential shuffles are now about 8 minutes in wasm, still longer than a deal. Remaining levers: a native prover (calibration next), and the split between fixed-base and variable-base multiplications.
