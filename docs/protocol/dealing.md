...
- **Seats.** `seat_owner` maps a seat to the player's key point, `ecMulGenerator(secret)`; a circuit acting for a seat proves knowledge of that secret (360 rows). `sit()` stands in for `seating.compact` until the two merge with the pot; seating's SHA-256 identity (3,900 rows per proof) switches to the point then.
- **Phases.** `idle -> keys -> shuffle -> holes -> playing`, or `-> aborted` from any waiting phase. `start_deal` may be called by anyone from idle, playing or aborted; the players of the deal are the seats occupied at that moment, fixed in `in_deal`, and the deck, joint key, keys, shares and counters reset. `post_key` moves to `shuffle` when every player has posted; `shuffle` runs in seat order (`turn`) and the last one moves to `holes`; `shares` counts each player's distinct shares of other players' hole positions and moves to `playing` when everyone has all of them. Board positions may be shared from `holes` on; gating them by street belongs to betting and is not here yet. One deal at a time; pipelining changes `start_deal` and nothing else.
- **Clock.** Compact has no current time, only `blockTimeGte` and `blockTimeLt` against the block. A caller states `now`; the contract checks `now <= block time < now + CLOCK_SLACK` (120 s) and sets the next deadline from it. Deadlines are in seconds of block time: `KEYS_SECONDS` 60, `SHUFFLE_SECONDS` 240 per shuffle, `HOLES_SECONDS` 90, each about three times the measured wasm proving time plus a block. A budget per player from the prover they declare comes with the wallet work.
...
- When a street's betting closes, the next street's board is **released**: each player still in posts `release`, and betting resumes once all have. If one or no player can still act, the whole remaining board releases at once; if none can, every share is already there and the deal goes straight to showdown. When everyone else has folded the pot goes to the last player and the deal is done.
...
- A deal that reaches the river's end waits in `showdown`; settling it by proof is the next piece of work. Escrow of a real asset (ADR 0007) replaces `sit`'s test chips later.
...
- **Betting clock**: 30 seconds plus a 30-second time bank per deal. The design session said expiry is check-else-fold; with shares travelling in the fold transaction that cannot be done for an absent player, so expiry aborts the deal on them (case 4 below) and the client auto-folds before the deadline instead. The time bank is not in the contract yet.
