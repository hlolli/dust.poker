...
- `src/live/` -- the Live table's chain side, no Three.js: `wallet.ts` the DApp connector handshake with Lace and the Bech32m address decoder; `ledger.ts` transactions (deploy, a call from a local circuit run) on `@midnightntwrk/ledger-v9`, with `ledger.test.ts` applying them to a ledger state in memory; `indexer.ts` the GraphQL reads; `live.ts` deploy and join through the wallet (its prover, its balancing). `ledger-shim.js` loads the ledger's wasm in the browser like `src/compact/onchain-runtime-shim.js` does the runtime's
