...
/** Joins the table on `chain` with `buy` chips; the chain's balancing adds buy plus bond of the table's asset. Returns the seat. */
...
/** Joins the table at `address` through the wallet. Returns the chain to play on and your seat. */
export async function joinTable(w: Wallet, profile: Profile, address: string, buy = 200n): Promise<{ chain: Chain; seat: Seat }> {
